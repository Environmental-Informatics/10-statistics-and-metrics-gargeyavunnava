...
Assignment 10: In this assignment, we take discharge data from tow rivers (wildcat creek and tippecanoe)
and calculate a series of statistical properties. No values cells are checked as gross errors. Once all the statistical properties 
have been calculated, the results are stored in two different .csv files.
...
def Calc7Q(Qvalues):
    ...
    mov_ave = Qvalues.rolling(7).mean()
    
    #calculating val7Q by selecting the min value from moving average calculated.
    val7Q = mov_ave.min()
...
def CalcExceed3TimesMedian(Qvalues):
    ...
    median_val_3_times = 3*(Qvalues.median())
    median3x = (Qvalues > median_val_3_times).sum()                               
...
def GetAnnualStatistics(DataDF):
    ...
    #column names of the dataframe (WYDataDF) contating annual water year data
    colnames = ['site_no', 'Mean Flow', 'Peak Flow', 'Median Flow', 'Coeff Var', 'Skew', 'Tqmean', 'R-B Index', '7Q', '3xMedian']

    #resample datetime index for the dataframe (WYDataDF) contating annual water year data using 'AS-OCT'
    #refer https://pandas.pydata.org/pandas-docs/stable/user_guide/timeseries.html for more info on how to resample over different time ranges
    
    #creating a dictionary to store each column in the data frame
    dic = {
        'site_no': DataDF.resample('AS-OCT').mean()['site_no'],
        'Mean Flow': DataDF['Discharge'].resample('AS-OCT').mean(),
        'Peak Flow': DataDF['Discharge'].resample('AS-OCT').max(),
        'Median Flow': DataDF['Discharge'].resample('AS-OCT').median(),
        'Coeff Var': (DataDF['Discharge'].resample('AS-OCT').std()/DataDF['Discharge'].resample('AS-OCT').mean())*100,
        'Skew': DataDF['Discharge'].resample('AS-OCT').apply(stats.skew), # skew from numpy
        'Tqmean': DataDF['Discharge'].resample("AS-OCT").apply(CalcTqmean), #from custom function defined above
        'R-B Index':  DataDF['Discharge'].resample("AS-OCT").apply(CalcRBindex),
        '7Q': DataDF['Discharge'].resample("AS-OCT").apply(Calc7Q),
        '3xMedian': DataDF['Discharge'].resample("AS-OCT").apply(CalcExceed3TimesMedian)
    }
    
    #Creating the final WYDataDF dataframe with all the data
    WYDataDF = pd.DataFrame(dic,columns = colnames)
...
def GetMonthlyStatistics(DataDF):
    ...
    #Creating the final MoDataDF dataframe with all the data
    MoDataDF = pd.DataFrame(dic,columns = colnames)
